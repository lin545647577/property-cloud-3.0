<template>
  <div class="content">
          ...
          <el-card class="box-card card-box">
            <div class="text item content-box">
              ...
              <div
                ...
                :key="item.id"
                class="notice-item"
                @click="toPage('/wisdom-oriented/notice/detail', item)"
              >
                <el-row :gutter="20">
                  <el-col :span="17">
    ...
    <weather style="position: fixed; top: 20px; z-index: 999"></weather>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initWS() {
      let that = this;
      this.ws = wsConnect(this.$store.state.auth.deviceUUID);
      // 监听socket连接
      ...
      this.ws.onmessage = (res) => {
        try {
          if (JSON.parse(res.data).business == "adv") {
            that.initAdvert();
          }
          // console.log("socket主动回调：：", JSON.parse(res.data));
        } catch {
