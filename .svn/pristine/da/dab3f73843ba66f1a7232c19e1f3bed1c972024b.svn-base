    ...
    >
      ...
      <el-table-column label="类型" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.type == 1 ? "公众号" : scope.row.type == 2 ? "小程序" : ""
        }}</template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="190">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" @click="updateInfo(scope.row.id)"
            >更新模板
          </el-button>
...
export default {
  ...
  methods: {
    ...
    handleUpdate() {
      ...
      this.updateInfo(this.ids);
    },
    updateInfo(ids) {
      updateMPFormWX(ids).then((res) => {
        this.$message({
          message: "操作成功",
          center: true,
          type: "success",
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportMPList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
