    ...
    <el-row v-if="!mapShow">
      <el-col :span="19">
        <div class="grid-content content-box">
          ...
          <div class="content my-scrollbar" id="contentDiv">
            ...
            <!--            回到顶部按钮-->
            <!--            <el-backtop target=".content" :bottom="55" :right="540">-->
            <!--              <div class="back-top">-->
      ...
      <el-col :span="5">
        <div class="grid-content">
          <search-box @handleOpen="handleMap" v-if="!mapShow" />
        </div>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    handleMap(data) {
      // console.log(data);
      // this.mapShow = true;
      this.$router.push({ path: "/wisdom-oriented/appointment" });
    },
    closeKeyBoard() {
      ...
      queryAnswerByWiseScreen(this.searchValue).then((res) => {
        ...
        for (let index in res) {
          if (res[index].content) {
            ...
            if (res[index].button) {
              res[index].title +=
                "<a style='color: #3A57FB;font-size: 22px;margin-left: 10px' href='" +
                res[index].button.url +
                "?deviceId=" +
                this.$store.state.auth.deviceUUID +
                "'>" +
                res[index].button.buttonName +
                "</a>";
            }
            this.conversationList.push({
              type: "a",
...
.content-box .content /deep/ .back-top {
  height: 100%;
  width: 100%;
