<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="nickname" label="微信名" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column prop="openid" label="微信openid" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="头像" show-overflow-tooltip>
        <template slot-scope="scope"
          ><el-avatar :size="30" :src="scope.row.headimgurl"></el-avatar
        ></template>
      </el-table-column>
      <el-table-column prop="registerIp" label="注册ip" show-overflow-tooltip>
      </el-table-column>
      <el-table-column
        prop="registerTime"
        label="注册时间"
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportWXUserList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "phone/WXUser/editModal.vue",
    ...
    queryList(currentPage) {
      ...
      queryWXUserList({
        ...
        for (let index in res.rows) {
          ...
          res.rows[index].nickname = decodeURI(res.rows[index].nickname);
        }
