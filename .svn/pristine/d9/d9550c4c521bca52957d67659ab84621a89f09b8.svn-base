<template>
  <div>
    <div class="search-box">
      <el-row :gutter="10" class="vertical-middle">
        <el-col :span="2" class="text-right vertical-middle">
          <div class="grid-content el-line-height">问题内容：</div>
        </el-col>
        ...
        <el-col :span="2" class="text-right vertical-middle">
          <div class="grid-content el-line-height">联系电话：</div>
        </el-col>
        <el-col :span="5" class="vertical-middle">
          <div class="grid-content">
        ...
        <el-col :span="2" class="text-right vertical-middle">
          <div class="grid-content el-line-height">反馈方式：</div>
        </el-col>
        <el-col :span="5" class="vertical-middle">
          <div class="grid-content">
            <el-select
            ...
            >
              ...
              <el-option label="不需要反馈" :value="0"></el-option>
    ...
    >
      ...
      <el-table-column prop="content" label="问题内容" shsow-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column label="反馈方式" show-overflow-tooltip>
        <template slot-scope="scope"
          >{{ findType(scope.row.feedbackType) }}
        </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column label="操作" width="160">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" @click="handleReply(scope.row)"
            >反馈
          </el-button>
  ...
  methods: {
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "order/question/user/editModal.vue",
    ...
    handleReply(row) {
      ...
      this.$confirm("确认反馈?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          replyUserQuestion(row).then(() => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
      ...
      // this.$refs["editModal"].show({
      //   view: "order/question/user/replyModal.vue",
      //   params: { id: row.id },
      //   title: "反馈用户问题",
      //   center: true,
      //   top: "5vh",
    ...
    handleDelete(ids) {
      this.$confirm("确认删除?", "提示", {
        ...
        .then(() => {
          deleteUserQuestion(ids).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
