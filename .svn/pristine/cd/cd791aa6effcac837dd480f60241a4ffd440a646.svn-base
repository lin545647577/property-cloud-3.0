<template>
  <el-row class="page-processType">
    <section>
      <el-row class="box-condition">
        <el-form size="mini" :inline="true" label-width="68px">
          <el-form-item label="分类">
            <el-select
              v-model="condition.kindCode"
              placeholder="请选择分类"
              clearable
            >
              <el-option
      ...
      <el-row class="box-table">
        ...
        >
          ...
          <el-table-column label="分类名称" prop="kindName" />
...
export default {
  ...
  methods: {
    queryList(pageNum, pageSize) {
      ...
      this.condition.kindCode = this.condition.kindCode || undefined;
      listProcessType(this.condition).then((res) => {
    ...
    handleView(row) {
      ...
      detailsProcessType(row.id).then((res) => {
        this.formData = res;
        this.dialogVisible = true;
      });
    },
    ...
    handleDelete(row) {
      this.$confirm("是否删除？", "删除")
        .then(() => {
          deleteProcessType(row.id || this.tableSelectedIds).then(() => {
