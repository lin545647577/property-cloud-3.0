<template>
  <div>
    <div class="search-box">
      <el-row :gutter="10" class="vertical-middle">
        <el-col :span="2" class="text-right vertical-middle">
          <div class="grid-content el-line-height">预约用户：</div>
        </el-col>
        ...
        <el-col :span="2" class="text-right vertical-middle">
          <div class="grid-content el-line-height">预约电话:</div>
        </el-col>
        <el-col :span="5" class="vertical-middle">
          <div class="grid-content">
    ...
    >
      ...
      <el-table-column prop="name" label="预约人" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="code" label="预约记录编码" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="phone" label="联系电话" show-overflow-tooltip>
      </el-table-column>
      <el-table-column
        prop="userOfReservation"
        label="预约用户"
        show-overflow-tooltip
      >
      </el-table-column>
      <el-table-column
        prop="timeOfReservation"
        label="预约时间"
        show-overflow-tooltip
      >
      </el-table-column>
      <el-table-column
        prop="nameOfRoomAssigned"
        label="分配房间名称"
        show-overflow-tooltip
      >
      </el-table-column>
      ...
      <el-table-column label="操作" width="120">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="text"
            @click="handleConfirm(false, scope.row)"
            >确认
          </el-button>
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportReservationList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "appointment/placeReservation/reservation/editModal.vue",
     ...
     * @param {Object} row
     */
    handleConfirm(checkInfo, row) {
      // console.log(index, row);
      this.$refs["editModal"].show({
        view: "appointment/placeReservation/reservation/audit/editModal.vue",
        params: { id: row.id },
        title: "确认",
        center: true,
        top: "5vh",
    ...
    handleDelete(ids) {
      this.$confirm("确认删除?", "提示", {
        ...
        .then(() => {
          // console.log(row);
          deleteReservation(ids).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
