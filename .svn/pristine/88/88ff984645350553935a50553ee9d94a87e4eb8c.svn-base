...
<script>
import {
  queryUserAdditionalList,
  exportAdditionalList,
  deleteAdditional,
  submitAdditional,
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportAdditionalList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "appointment/placeReservation/additional/editModal.vue",
    ...
    handleSubmit(row) {
      if (row.status === "0" || row.status === "2") {
        submitAdditional(row.id).then((res) => {
          this.$message({
            message: "操作成功",
            center: true,
            type: "success",
    ...
    handleEdit(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        ...
        params: {
          checkInfo: checkInfo,
          id: row.id,
          axiosUrl: "@/api/appointment.js",
          queryFormData: "queryAdditional",
          queryFormElement: "queryForm",
          editFormData: "editAdditional",
          insertFormData: "insertAdditional",
        },
        title: "编辑附加服务",
        center: true,
        top: "5vh",
