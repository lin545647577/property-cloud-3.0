<template>
  <div>
    ...
    <div class="btn-box">
      ...
      <el-button size="small" class="control-btn" type="danger" @click="handleDeletes"
      >删除
      </el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column prop="amountYuan" label="金额">
      </el-table-column>
      <el-table-column label="状态" width="60">
        <template slot-scope="scope">{{ scope.row.status == "build" ? "创建" : "发布" }}</template>
      </el-table-column>
      <el-table-column label="支付状态">
        <template slot-scope="scope">{{ scope.row.payStatus ? "已支付" : "未支付" }}</template>
      </el-table-column>
      <el-table-column label="操作" width="150">
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="handleEdit(false, scope.row)">修改金额</el-button>
          <el-button type="text" size="mini" @click="handlePay( scope.row)">支付</el-button>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      // console.log(ids);
      deleteOwnerBill(row.id).then((res) => {
        this.$message({
          message: "操作成功",
          center: true,
          type: "success",
      ...
      queryOwnerBillList(
          {
            ...
            referenceMemberId: this.params.referenceMemberId
          }).then(
          res => {
            // console.log(res);
