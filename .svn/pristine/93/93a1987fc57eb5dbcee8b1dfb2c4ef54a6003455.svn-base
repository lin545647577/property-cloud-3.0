<template>
  <div>
    ...
    >
      ...
      <el-table-column label="公告分类" show-overflow-tooltip>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" @click="checkPublish(scope.row)"
            >发布
          </el-button>
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportNoticeList({ theme: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
