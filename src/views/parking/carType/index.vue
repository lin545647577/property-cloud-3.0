<template>
  <div>
    ...
    >
      ...
      <el-table-column label="车场" show-overflow-tooltip prop="carParkName">
      </el-table-column>
      <el-table-column
        prop="deviceKey"
        label="车辆类型编号"
...
import {
  ...
  exportCarParkTypeConfigList,
  deleteCarParkTypeConfig,
} from "@/api/parking.js";
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportCarParkTypeConfigList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "parking/carType/editModal.vue",
    ...
    handleDelete(ids) {
      this.$confirm("确认删除?", "提示", {
        ...
        .then(() => {
          deleteCarParkTypeConfig(ids).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
