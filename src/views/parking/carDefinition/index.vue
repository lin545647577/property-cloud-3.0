<template>
  <div>
    <div class="search-box">
      <el-row :gutter="10" class="vertical-middle">
        <el-col :span="2" class="text-right vertical-middle">
          <div class="grid-content el-line-height">车牌：</div>
        </el-col>
        <el-col :span="3" class="vertical-middle">
          <div class="grid-content">
            <el-input v-model="searchCarNo" placeholder="请输入内容"></el-input>
          </div>
        </el-col>
        <el-col :span="2" class="text-right vertical-middle">
          <div class="grid-content el-line-height">车主：</div>
        </el-col>
        <el-col :span="3" class="vertical-middle">
          <div class="grid-content">
    ...
    >
      ...
      <el-table-column prop="carNo" label="车牌号" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="carType" label="车辆类型" show-overflow-tooltip>
        <!-- <template slot-scope="scope" v-if="scope.row.type">
          {{findCarType(scope.row.type)}}
        </template> -->
      </el-table-column>
      <el-table-column prop="name" label="车主" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column label="所属业主" show-overflow-tooltip prop="ownerName">
      </el-table-column>
...
<script>
import {
  queryCarDefinitionList,
  exportCarDefinitionList,
  deleteCarDefinition,
} from "@/api/parking.js";
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportCarDefinitionList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "parking/carDefinition/editModal.vue",
    ...
    handleDelete(ids) {
      this.$confirm("确认删除?", "提示", {
        ...
        .then(() => {
          deleteCarDefinition(ids).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
