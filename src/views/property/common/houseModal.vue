<template>
  <div>
    <div class="search-box">
      <el-row :gutter="10" class="vertical-middle">
        <el-col :span="4" class="text-right vertical-middle">
          <div class="grid-content el-line-height">房屋名称：</div>
        </el-col>
    ...
    >
      ...
      <el-table-column label="物业名称" show-overflow-tooltip>
        <template slot-scope="scope">{{ scope.row.residenceName }}</template>
      </el-table-column>
      <el-table-column label="楼宇名称" show-overflow-tooltip>
        <template slot-scope="scope">{{ scope.row.buildingName }}</template>
      </el-table-column>
      <el-table-column prop="name" label="房屋名称" show-overflow-tooltip>
      </el-table-column>
...
<script>
import { queryHouseList } from "@/api/property.js";
export default {
  name: "houseModal",
  data() {
    return {
