...
export default {
    ...
    initNoticeList() {
      queryNoticeList({
        ...
        for (let index in res.rows) {
          res.rows[index].startDate = dateFormat(
            "MM-dd",
            new Date(res.rows[index].startDate)
          );
        }
  ...
  computed: {
    ...
    loginUser() {
      if (this.$store.state.auth.loginInfo) {
        return this.$store.state.auth.loginInfo.user;
      } else {
        return {};
      }
    },
