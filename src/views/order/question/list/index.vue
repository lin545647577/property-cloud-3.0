<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="explain" label="问题说明" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="常见问题" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.commonStatus === 0 ? "否" : "是"
        }}</template>
      </el-table-column>
      <el-table-column prop="typeName" label="问题分类" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="点赞数量" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.prosNum ? scope.row.prosNum : 0
        }}</template>
      </el-table-column>
      <el-table-column label="点踩数量" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.consNum ? scope.row.consNum : 0
        }}</template>
      </el-table-column>
  ...
  methods: {
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "order/question/list/editModal.vue",
