<template>
  <div>
    ...
    >
      ...
      <el-table-column label="匿名" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.orNotName === 0 ? "否" : "是"
        }}</template>
      </el-table-column>
      <el-table-column prop="suggestContent" label="内容" show-overflow-tooltip>
      </el-table-column>
      <el-table-column
        prop="suggestLocation"
        label="位置"
        show-overflow-tooltip
      >
      </el-table-column>
      ...
      <el-table-column label="投诉类型" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.suggestType == "0" ? "建议" : "投诉"
        }}</template>
      </el-table-column>
      <el-table-column label="投诉状态" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.suggestStatus === 0 ? "未回复" : "已回复"
        }}</template>
      </el-table-column>
      <!--      <el-table-column label="是否推送消息" show-overflow-tooltip>-->
      <!--        <template slot-scope="scope">{{-->
      <!--          scope.row.send === 0 ? "否" : "是"-->
      <!--        }}</template>-->
      <!--      </el-table-column>-->
      <el-table-column label="推送类型" show-overflow-tooltip>
        <template slot-scope="scope">{{
      ...
      <el-table-column
        ...
        show-overflow-tooltip
      ></el-table-column>
      <el-table-column prop="bakContent" label="回复说明" show-overflow-tooltip>
      </el-table-column>
  ...
  methods: {
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "order/suggestion/editModal.vue",
