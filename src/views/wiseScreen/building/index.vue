    ...
    <div class="content-box">
      ...
      <div class="building-content">
        <div class="building-box" v-for="item in floorList" :key="item.id">
          <p class="floor">
            <span class="sp-box" v-for="c in item.name" :key="c">{{ c }}</span>
          </p>
          <p class="floor-en">
            <span class="sp-box" v-for="c in 'GROUND FLOOR'" :key="c">{{
              c
            }}</span>
          </p>
          <el-row
          ...
          >
            <el-col
              ...
              :key="com.fid"
              ><div class="grid-content">
                <span v-for="c in com.name" :key="c" class="char-box">
                  <span class="sp-box" v-for="w in c" :key="w">{{ w }}</span>
                </span>
              </div></el-col
            >
          </el-row>
          <el-row type="flex" class="row-bg" :gutter="50" v-else>
            <el-col :span="8" key="com.id"
              ><div class="grid-content">
                <span class="char-box">
                  <span class="sp-box" v-for="w in '暂无信息'" :key="w">{{
                    w
                  }}</span>
                </span>
              </div></el-col
...
export default {
  ...
  methods: {
    ...
    getCompanyList(fid) {
      return this.companyList.find((item) => item.fid === fid);
    },
  },
  mounted() {
...
.common-box .content-box .sp-box {
  line-height: 45px;
  background-image: url("../../../assets/img/wiseScreen/2.0/buiildcharBox.png");
  padding: 3px 5px;
  /*margin-right: 5px;*/
...
.common-box .content-box .building-box .char-box {
...
.common-box .content-box .building-box .floor-en {
  font-size: 10px;
  margin-bottom: 2.5%;
}

.common-box .content-box .building-box .floor-en .sp-box {
  line-height: 30px;
  padding: 0 3px;
}
</style>
