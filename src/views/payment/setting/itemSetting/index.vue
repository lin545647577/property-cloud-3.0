        ...
        <div class="grid-content">
          <div class="search-box">
            <el-row :gutter="10" class="vertical-middle">
              <el-col :span="2" class="text-right vertical-middle">
                <div class="grid-content el-line-height">费用项目：</div>
              </el-col>
          ...
          >
            ...
            <el-table-column
              ...
              width="60"
            ></el-table-column>
            <el-table-column prop="name" label="费用项目" show-overflow-tooltip>
            </el-table-column>
            <el-table-column label="计算公式" prop="formula"> </el-table-column>
            <el-table-column prop="amount" label="单价(元)"> </el-table-column>
            <el-table-column label="费用类型">
              <template slot-scope="scope">{{
                findType(scope.row.type)
              }}</template>
            </el-table-column>
...
<script>
import { queryItemList, exportItemList, deleteItem } from "@/api/payment.js";
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportItemList({ name: this.searchName }).then((res) => {
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "payment/setting/itemSetting/editModal.vue",
        params: {
          checkInfo: checkInfo,
          id: row.id,
