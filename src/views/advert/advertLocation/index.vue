<template>
  <div>
    <div class="search-box" v-if="searchList.length > 0">
      <my-search-box
        :searchList="searchList"
        @mySearch="handleSearch"
      ></my-search-box>
      <!--      <my-search-box :searchList="searchList"></my-search-box>-->

      <!--      <el-row :gutter="10" class="vertical-middle">-->
      <!--        <el-col :span="2" class="text-right vertical-middle">-->
      <!--          <div class="grid-content el-line-height">广告名称：</div>-->
      <!--        </el-col>-->
      <!--        <el-col :span="5" class="vertical-middle">-->
      <!--          <div class="grid-content">-->
      <!--            <el-input-->
      <!--              v-model="searchAdvName"-->
      <!--              placeholder="请输入内容"-->
      <!--            ></el-input>-->
      <!--          </div>-->
      <!--        </el-col>-->
      <!--        <el-col :span="2" class="text-right vertical-middle">-->
      <!--          <div class="grid-content el-line-height">设备名称：</div>-->
      <!--        </el-col>-->
      <!--        <el-col :span="5" class="vertical-middle">-->
      <!--          <div class="grid-content">-->
      <!--            <el-input-->
      <!--              v-model="searchDeviceName"-->
      <!--              placeholder="请输入内容"-->
      <!--            ></el-input>-->
      <!--          </div>-->
      <!--        </el-col>-->
      <!--        <el-col :span="3" class="vertical-middle">-->
      <!--          <div class="grid-content text-right">-->
      <!--            <el-button-->
      <!--              size="small"-->
      <!--              class="search-btn"-->
      <!--              type="primary"-->
      <!--              @click="queryList((currentPage = 1))"-->
      <!--              >搜索-->
      <!--            </el-button>-->
      <!--          </div>-->
      <!--        </el-col>-->
      <!--      </el-row>-->
    </div>

    <table-header-btn
      :insertBtn="true"
      :exportBtn="true"
      :deleteBtn="true"
      @handleInsert="handleInsert(false)"
      @handleExport="handleExport"
      @handleDeletes="handleDeletes"
    ></table-header-btn>
    <el-table
      border
      ref="multipleTable"
      :data="tableData"
      tooltip-effect="dark"
      style="width: 100%"
      @selection-change="handleSelectionChange"
    >
      <el-table-column type="selection"></el-table-column>
      <el-table-column label="序号" prop="index" width="60"></el-table-column>
      <el-table-column
        prop="tygxAdvDefinition.advName"
        label="广告名称"
        show-overflow-tooltip
      >
      </el-table-column>
      <el-table-column label="广告状态" show-overflow-tooltip>
        <template slot-scope="scope">{{
          findStatus(scope.row.advState)
        }}</template>
      </el-table-column>
      <!--      <el-table-column-->
      <!--        prop="locationName"-->
      <!--        label="广告位置"-->
      <!--        show-overflow-tooltip-->
      <!--      >-->
      <!--      </el-table-column>-->
      <el-table-column label="默认广告" show-overflow-tooltip>
        <template slot-scope="scope">{{
          scope.row.orNotDefault === 0 ? "否" : "是"
        }}</template>
      </el-table-column>
      <el-table-column prop="deviceName" label="设备名称" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="操作" width="160">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
            @click="handleCheck(true, scope.row)"
            >查看
          </el-button>
          <el-button
            size="mini"
            type="text"
            @click="handleEdit(false, scope.row)"
            >编辑
          </el-button>
          <el-button
            v-if="scope.row.advState === 1"
            size="mini"
            type="text"
            @click="handleOffLine(scope.row)"
            >作废
          </el-button>
          <el-button
            v-if="scope.row.advState === 0"
            size="mini"
            type="text"
            @click="handlePublish(scope.row)"
            >审核
          </el-button>
          <el-button size="mini" type="text" @click="handleDelete(scope.row.id)"
            >删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>

    <div class="block pagination-center">
      <el-pagination
        @size-change="handleSizeChange"
        @current-change="handleCurrentChange"
        :current-page="currentPage"
        :page-sizes="pageSizes"
        :page-size="pageSize"
        layout="total, sizes, prev, pager, next, jumper"
        :total="total"
      >
      </el-pagination>
    </div>
    <li-modal style="min-width: 1200px" ref="editModal" />
  </div>
</template>

<script>
import {
  queryAdvDeviceLocationList,
  exportAdvDeviceLocationList,
  deleteAdvDeviceLocation,
  publishAdvInfo,
  queryAdvDeviceLocation,
  offLineAdvInfo,
} from "@/api/advert.js";
import { resolveBlob } from "@/utils/util.js";
import tableHeaderBtn from "@/components/tableHeaderBtn";
import searchBox from "@/components/searchBox";
import { wsConnect } from "@/api/common";
import eventBus from "@/utils/eventBus";

export default {
  name: "advertLocation",
  components: {
    "table-header-btn": tableHeaderBtn,
    "my-search-box": searchBox,
  },
  data() {
    return {
      tableData: [],
      multipleSelection: [],
      currentPage: 1,
      pageSizes: [10, 20, 50, 100],
      pageSize: 10,
      total: 0,
      fullscreenLoading: false,
      ids: "",

      searchList: [
        {
          type: "input",
          title: "广告名称",
          key: "advName",
          holder: "请输入",
        },
        {
          type: "input",
          title: "设备名称",
          key: "deviceName",
          holder: "请输入",
        },
      ], //搜索元素

      status: [
        {
          id: 0,
          name: "未发报",
        },
        {
          id: 1,
          name: "已发布",
        },
        {
          id: 2,
          name: "下发中",
        },
        {
          id: 3,
          name: "过期",
        },
        {
          id: 4,
          name: "作废",
        },
        {
          id: 5,
          name: "下发失败",
        },
      ],
    };
  },
  methods: {
    findStatus(id) {
      let res = this.status.find((item) => {
        if (item.id === id) {
          return item;
        }
      });
      return res ? res.name : "";
    },
    /**
     * 导出信息
     */
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          this.fullscreenLoading = true;
          exportAdvDeviceLocationList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
            });
            resolveBlob(res, res.type);
            this.fullscreenLoading = false;
          });
        })
        .catch(() => {
          this.$message({
            type: "info",
            message: "已取消",
          });
        });
    },
    //表格选择
    handleSelectionChange(val) {
      this.multipleSelection = val;
      let tempArr = [];
      for (let index in val) {
        tempArr.push(val[index].id);
      }
      this.ids = tempArr.toString();
      // console.log("表格选择",this.ids);
    },
    /**
     * 删除
     * @param {String} ids
     */
    handleDeletes() {
      // console.log("ids:", this.ids);
      if (!this.ids) {
        this.$message({
          message: "请勾选内容",
          center: true,
          type: "warning",
        });
        return;
      }
      this.handleDelete(this.ids);
    },

    /**
     * 查看操作
     * @param {boolean} checkInfo
     * @param {Object} row
     */
    handleCheck(checkInfo, row) {
      // console.log(row);
      this.$refs["editModal"].show({
        view: "advert/advertLocation/editModal.vue",
        params: { checkInfo: checkInfo, id: row.id },
        title: "查看广告推送信息",
        center: true,
        top: "5vh",
        hideSubmitButton: true,
        hideCancelButton: true,
      });
    },

    /**
     *modal回调
     */
    modalCallFunction() {
      this.queryList(this.currentPage);
      this.$refs["editModal"].hide();
    },

    /**
     * 编辑操作
     * @param {boolean} checkInfo
     * @param {Object} row
     */
    handleEdit(checkInfo, row) {
      // console.log(index, row);
      this.$refs["editModal"].show({
        view: "advert/advertLocation/editModal.vue",
        params: { checkInfo: checkInfo, id: row.id },
        title: "编辑广告推送信息",
        center: true,
        top: "5vh",
        hideSubmitButton: true,
        hideCancelButton: true,
        submit: this.modalCallFunction,
      });
    },
    /**
     * 作废操作
     * @param {Object} row
     */
    handleOffLine(row) {
      if (row.advState === 1) {
        this.$confirm("是否作废该广告?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
          .then(() => {
            let that = this;
            queryAdvDeviceLocation(row.id).then((res) => {
              // console.log(res);
              offLineAdvInfo(row.id, res.deviceUuid).then(() => {
                that.$message({
                  message: "操作成功",
                  center: true,
                  type: "success",
                });
                that.queryList(that.currentPage);
              });
            });
          })
          .catch(() => {
            this.$message({
              type: "info",
              message: "已取消",
            });
          });
      } else {
        this.$message({
          message: "该广告未发布",
          center: true,
          type: "warning",
        });
        return;
      }
    },

    /**
     * 推送操作
     * @param {Object} row
     */
    handlePublish(row) {
      // console.log(index, row);
      if (row.advState === 1) {
        this.$message({
          message: "该广告已经发布",
          center: true,
          type: "warning",
        });
        return;
      }

      this.$confirm("是否发布该广告?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          publishAdvInfo(row.id).then(() => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
            });
            this.queryList(this.currentPage);
          });
        })
        .catch(() => {
          this.$message({
            type: "info",
            message: "已取消",
          });
        });
    },

    /**
     * 新增操作
     * @param {boolean} checkInfo
     */
    handleInsert(checkInfo) {
      // console.log(index, row);
      this.$refs["editModal"].show({
        view: "advert/advertLocation/editModal.vue",
        params: { checkInfo: checkInfo },
        title: "新增广告推送信息",
        center: true,
        top: "5vh",
        hideSubmitButton: true,
        hideCancelButton: true,
        submit: this.modalCallFunction,
      });
    },
    /**
     * 删除
     * @param index
     * @param {Object} row
     */
    handleDelete(ids) {
      this.$confirm("确认删除?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          deleteAdvDeviceLocation(ids).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
            });
            if (this.total % this.pageSize === 1 && this.currentPage > 1) {
              this.currentPage--;
            }
            this.queryList(this.currentPage);
          });
        })
        .catch(() => {
          this.$message({
            type: "info",
            message: "已取消删除",
          });
        });
    },

    //每页显示条数
    handleSizeChange(val) {
      // console.log(`每页 ${val} 条`);
      this.pageSize = val;
      this.queryList((this.currentPage = 1));
    },

    // 分页
    handleCurrentChange(val) {
      this.queryList((this.currentPage = val));
    },

    handleSearch(args) {
      // console.log("回传：", args);
      this.queryList((this.currentPage = 1), args);
    },

    queryList(currentPage, args) {
      let condition = {
        pageNum: currentPage,
        pageSize: this.pageSize,
      }; //查询条件
      if (args) {
        Object.assign(condition, args);
      }
      // console.log(condition);

      queryAdvDeviceLocationList(condition).then((res) => {
        // console.log(res);
        for (let index in res.rows) {
          // console.log(res.rows[index].tygxAdvDefinition.advState);
          res.rows[index].index =
            parseInt(index) + 1 + (this.currentPage - 1) * this.pageSize;
        }
        this.tableData = res.rows;
        this.total = res.total;
      });
    },
  },
  mounted() {
    this.queryList(this.currentPage);
    // eventBus.$on("mySearch", (args) => {
    //   this.queryList((this.currentPage = 1), args);
    // });
  },
};
</script>

<style scoped></style>
