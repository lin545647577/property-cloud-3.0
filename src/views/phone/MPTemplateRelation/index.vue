<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="appName" label="公众号" show-overflow-tooltip>
      </el-table-column>
...
<script>
import {
  queryTemplateMpConfigList,
  exportTemplateMpConfigList,
  deleteTemplateMpConfig,
} from "@/api/phone.js";
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否导出全部数据?", "提示", {
        ...
        .then(() => {
          ...
          exportTemplateMpConfigList({ name: this.searchName }).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
    ...
    handleCheck(checkInfo, row) {
      ...
      this.$refs["editModal"].show({
        view: "phone/MPTemplateRelation/editModal.vue",
    ...
    handleDelete(ids) {
      this.$confirm("确认删除?", "提示", {
        ...
        .then(() => {
          deleteTemplateMpConfig(ids).then((res) => {
            this.$message({
              message: "操作成功",
              center: true,
              type: "success",
